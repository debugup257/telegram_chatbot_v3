...
def start(message):
    uid = message.chat.id
    users_state[uid] = {"tracker":0,"name":None,"email": None, "pan":None, "location":None, "exp":None, "edu":None,"role":None ,"eligible_roles":[],"shuffled_list":[],"final_basic_questions":[],"asked_questions":[],"technical":{}}
...
def introduction(message,nlp=nlp,users_state=users_state,db=db):
    ...
    if users_state[uid]["tracker"]==8:
        ...
        if len(users_state[uid]["shuffled_list"])>0:
            ...
            # uploading the data 
            available_id_in_db=(db.fetch_column("id"))
            print(available_id_in_db)
            available_id_in_db = list(val for tuple in available_id_in_db for val in tuple)  
            uid_unique=uid
            while (str(uid_unique) in available_id_in_db)==True:
                print("while loop")
                uid_unique=int(uid_unique)+1000000000000
                uid_unique=str(uid_unique)
                print(uid_unique)
            db.insert_applicant(users_state[uid],uid_unique)
            db.upload_data(users_state[uid]["technical"],uid_unique,users_state[uid]["role"])
...
def handle_callback(call):
    ...
    users_state[uid]["role"]=call.data
    print("call_back")
